...
generateButton.addEventListener('click', () => {
    if (!generateBreak) {
        ...
        if (varTextarea != 2) {
            ...
            const iValue = removeNumbers.value.trim();
            // Валидация
            const removeNumbersArray = extractNumbers(iValue);
        ...
        else if (varTextarea == 2) {//числа из списка
        
            let randomNumberDo
            const iValue = outputList.value.trim();
            // Валидация
            const outputNumbersArray = extractNumbers(iValue);
            // Преобразуем числа в массив и исключаем дубликаты
...
function showNumbers(randomNumberDo) {
    generateBreak = true;
    let msK = 100;
    randomNumber.innerHTML = '';
    for (let i = 0; i < randomNumberDo.toString().length; i++) {
        setTimeout(() => {
            randomNumber.innerHTML += randomNumberDo.toString()[i];//добавляем по 1 цифре
        }, i * msK)
    }
    setTimeout(() => {
        generateBreak = false;
    }, (randomNumberDo.toString().length + 1) * msK)
}
...
function runnerFind() {
    ...
    if (Math.round(RunnerDot) == points[0]) {
        ...
        minRange.classList.remove('disabled');
        maxRange.classList.remove('disabled');
        ...
        minRange.classList.add('disabled');
        maxRange.classList.add('disabled');
